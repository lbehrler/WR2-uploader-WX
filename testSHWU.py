...
try:
    ...
    sense.show_message('Go!', text_colour=[255, 255, 0], back_colour=[0, 0, 255])
    # clear the screen
    sense.clear()
except:
...
def disp_sh(temp,hum):
    # Send the local temp/humidity data to the SenseHat
    try:
        logging.info('Initializing the Sense HAT client')
        shMsg= "T: " + temp + " H: " + hum
   ...
   #sys.stdout.write('Made it to processing step. \n')
	...
	pulse += 1
        ...
        if (( sLine.find('F007TH') != -1) or ( sLine.find('F016TH') != -1)):
            ...
            disp_sh(indtemp_str,indhumidity_str)
